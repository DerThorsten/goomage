...
class PlainGoo(GooBase):
    ...
    @staticmethod
    def kivy_draw_tentative(root, position, insert_info):
        insert_type = insert_info.insert_type
        alpha = 0.5
        if insert_type == InsertType.AS_GOO:
    
            
            radius = PlainGoo.radius
            size = (radius*2, radius*2)
            pos = list(position)
            pos[0] -= radius
            pos[1] -= radius

            Line(points=[position, tuple(insert_info.info['goo_a'].body.position)], 
                width=0.15, color=Color(0.5,0.5,1,alpha))
            Line(points=[position, tuple(insert_info.info['goo_b'].body.position)], 
                width=0.15, color=Color(0.5,0.5,1,alpha))
            ...
            Line(points=[
                    tuple(insert_info.info['goo_a'].body.position), 
                    tuple(insert_info.info['goo_b'].body.position)
                ], 
                width=0.15, color=Color(0.5,0.5,1,alpha))
